<x-splade-modal class="font-main">
    <h1 class="text-2xl font-bold mb-4">{{trans('tomato-admin::global.crud.edit')}} {{ trans('tomato-notifications::global.templates.single') }} #{{$model->id}}</h1>
    ...
    <x-splade-form class="flex flex-col space-y-4" action="{{route('admin.notifications-templates.update', $model->id)}}" method="post" :default="$model">
        <x-splade-file name="image" :label="trans('tomato-notifications::global.templates.image')" preview  filepond/>
        ...
        <x-splade-submit label="{{trans('tomato-admin::global.crud.update')}} {{ trans('tomato-notifications::global.templates.single') }}" :spinner="true" />
    </x-splade-form>
</x-splade-modal>
